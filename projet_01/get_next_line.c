...
/*   Updated: 2020/02/20 05:27:29 by amepocch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_freetmp(char **line, char **tmp)
{
	*line = ft_strdup("");
	if (*tmp)
		free(*tmp);
...
int			ft_else(char **line, char **tmp)
{
	*line = *tmp;
	*tmp = NULL;
	free(*tmp);
...
int			get_next_line(int fd, char **line)
{
	...
	if (nb_char == -1)
		if ((ft_freetmp(line, &tmp[fd])) == -1)
			return (-1);
	if (len != -1)
		ft_saveline(line, &tmp[fd], len);
	else
	{
		if ((ft_else(line, &tmp[fd])) == 0)
			return (0);
	}
	return (1);
}
